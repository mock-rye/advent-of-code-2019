...
int process(string wire1, string wire2){
	int ht, out = 10000000, stepCounter = 0, aux;
	pii currC = {0,0}, cMod;
	map<unsigned int, vector<pii>> tracemap;
	for(int i = 0; i < wire1.length(); ++i){
		ht = 0;
		for(int j = 1; j < wire1.length()-i; ++j){
			if(wire1[i+j] == ','){
				aux = i + j;
				break;
			}
			ht = ht*10 + wire1[i+j] - 48;
		}
		cMod = mod(wire1[i]);
		for(int j = 0; j < ht; ++j){
			currC = currC + cMod;
			++stepCounter;
			tracemap[currC.first].push_back(make_pair(currC.second, stepCounter));
		}
		i = aux;
	}
	currC = {0,0};
	...
	for(int i = 0; i < wire2.length(); ++i){
		ht = 0;
		for(int j = 1; j < wire2.length()-i; ++j){
			if(wire2[i+j] == ','){
				aux = i + j;
				break;
			}
			ht = ht*10 + wire2[i+j] - 48;
		}
		cMod = mod(wire2[i]);
		for(int j = 0; j < ht; ++j){
			currC = currC + cMod;
			++stepCounter;
			if(isinvect(tracemap[currC.first], currC.second)){
				out = min(stepCounter + tracemap[currC.first][isinvect(tracemap[currC.first], currC.second)-1].second, out);
			}
		}
		i = aux;
	}
