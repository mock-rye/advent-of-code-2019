...
int isinvect(vector<pii> vi, int find){
	for(int i = 0; i != vi.size(); ++i){
		if(vi[i].first == find) return i+1;
	}
	return 0;
}
...
int main(){
	...
	for(int i = 0; i < a.length(); ++i){
		...
		for(int j = 0; j < ht; ++j){
			currC = currC + cMod;
			++stepCounter;
			if(isinrange(currC)) tracemap[currC.first].push_back(make_pair(currC.second, stepCounter));
		}
		i = aux;
	}
	cout << "<<<<<<<<<<<<<<<<<<<<<<<<<" << endl << endl << "<<<<<<<<<<<<<<<<<<<<<<<<<" << endl;
	...
	for(int i = 0; i < b.length(); ++i){
		...
		for(int j = 0; j < ht; ++j){
			currC = currC + cMod;
			++stepCounter;
			if(isinrange(currC)) if(isinvect(tracemap[currC.first], currC.second)){
				cout << 'A' << endl;
				intersections.push_back(stepCounter + tracemap[currC.first][isinvect(tracemap[currC.first], currC.second)-1].second);
				cout << currC.first << ' ' << currC.second << endl;
			}
		}
		i = aux;
	}
