...
int main(){
	...
	pair<int,int> pout;
	for(int i = 0; i < intersections.size(); ++i){
		if(out > max(intersections[i].first, -(intersections[i].first)) + max(intersections[i].second, -(intersections[i].second))){
			out = max(intersections[i].first, -(intersections[i].first)) + max(intersections[i].second, -(intersections[i].second));
			pout = intersections[i];
		}
	}
	cout << out << endl;
	cout << pout.first << ' ' << pout.second << endl;
	return 0;
}
